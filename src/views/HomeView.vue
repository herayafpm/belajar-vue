<template>
  ...
  <div class="container">
    ...
    <div v-if="consultation.length === 0 || consultation.status !== 'accepted'" class="alert alert-warning">Your Consultation must be accepted by doctor to get vaccine.</div>
    <div v-else>
      <div class="row">
         <div class="col-md-3">
            <div class="card">
              ...
              <div class="card-body p-0">
                <div class="p-2" v-if="vaccinations.first.status === undefined">
                  <RouterLink to="/list_spots/1" class="decoration">+ <span style="text-decoration:underline">Register Vaccination</span></RouterLink>
                </div>
                <table v-else class="table table-striped">
                  <tbody>
                    <tr>
                      <td class="p-2">Status</td>
                      <td><span class="badge bg-info" style="text-transform:capitalize">{{vaccinations.first.status}}</span></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      <td v-if="vaccinations.first.spot !== undefined">{{vaccinations.first.spot.name}}</td>
                      <td v-else>-</td>
                    </tr>
                    <tr>
                      <td class="p-2">Vaccine</td>
                      <td v-if="vaccinations.first.vaccine != null">{{vaccinations.first.vaccine.name}}</td>
                      <td v-else>-</td>
                    </tr>
                    <tr>
                      <td class="p-2">Vaccinator</td>
                      <td v-if="vaccinations.first.vaccinator != null">{{vaccinations.first.vaccinator.name}}</td>
                      <td v-else>-</td>
                    </tr>
                  </tbody>
                </table>
            ...
            <div class="card">
              ...
              <div class="card-body p-0">
                <div class="p-2" v-if="vaccinations.second.status === undefined">
                  <RouterLink to="/list_spots/2" class="decoration">+ <span style="text-decoration:underline">Register Vaccination</span></RouterLink>
                </div>
                <table v-else class="table table-striped">
                  <tbody>
                    <tr>
                      <td class="p-2">Status</td>
                      <td><span class="badge bg-info" style="text-transform:capitalize">{{vaccinations.second.status}}</span></td>
                    </tr>
                    <tr>
                      <td class="p-2">Date</td>
                    ...
                    <tr>
                      <td class="p-2">Spot</td>
                      <td v-if="vaccinations.second.spot !== undefined">{{vaccinations.second.spot.name}}</td>
                      <td v-else>-</td>
                    </tr>
                    <tr>
                      <td class="p-2">Vaccine</td>
                      <td v-if="vaccinations.second.vaccine != null">{{vaccinations.second.vaccine.name}}</td>
                      <td v-else>-</td>
                    </tr>
                    <tr>
                      <td class="p-2">Vaccinator</td>
                      <td v-if="vaccinations.second.vaccinator != null">{{vaccinations.second.vaccinator.name}}</td>
                      <td v-else>-</td>
                    </tr>
                  </tbody>
                </table>
  ...
  export default{
    ...
    methods: {
      getVaccinations(){
        ...
        xhr.open("GET",this.$apiURL+'vaccinations?token='+this.$attrs.modelValue.token)
        xhr.send(null)
      },
      getConsultation(){
        var xhr = new XMLHttpRequest();
        ...
        xhr.open("GET",this.$apiURL+'consultations?token='+this.$attrs.modelValue.token)
        xhr.send(null)
      }
    },
    data(){
